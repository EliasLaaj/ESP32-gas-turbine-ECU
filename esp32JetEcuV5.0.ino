...
// Constants
const int MIN_RPM = 42000;                      // Minimum rpm before EPO activates.
const int RPM_LIMIT = 100000;                   // Maximum safe operating rpm.
const int FLAME_THRESHOLD = 1600;               // Raw value from flame sensor to detect flame. 0 if no flame, ~1000 with spark and 4095 with flame.
...
const int RELIGHT_ACTIVATION_THRESHOLD = 32000; // Rpm needed which after relight is in use.
const int START_FUEL_MULTIPLIER = 1.25;         // Fuel pump multiplier for ignition and initial ramp up.
...
bool firstShutdownInStartup = 0;

// Flags for function presets
bool skipSafetyChecks = 0;
bool enableSensorData = 1;
...
float readPressure();
bool flameDetector();
bool checkStopSwitch();

void setup() {
  SerialBT.begin("JetECU"); // Bluetooth device name
  Serial2.begin(115200, SERIAL_8N1, RX2, TX2); // "S:#number1-11#", "T:TOT", "R:RPM"
...
void startup() {
  ...
  setOilValue(3.4, 0);
  setStarterESC(70, 0.75);
  if (checkStopSwitch()) {
    return;
  }

  runningState = 1;
  digitalWrite(igniterPin, LOW); // Make sure that igniter is off for safety reasons
...
void shutdown() {
  ...
  setStarterESC(15, 0.8); // Cool turbine down
  if (extendedCoolDown){ 
    shutdownTimer = millis();
    while (avgTOT > 150 && millis() - shutdownTimer < 90000) {
      sleep(1);

      if (digitalRead(StopSwitch) == 0 && digitalRead(StartSwitch) == 0) { // Skip cooldown if stop switch is active and start switch is pressed over 1s
        delay(1000);
  ...
  sleep(5); // Vent combustion chamber
  setStarterESC(0, 1);
  shutdownTimer = millis();
...
bool checkStopSwitch() { // Checks if stopswitch is activated or flamout has occured during startup

  if (!firstShutdownInStartup) { // If shutDown has run allready
    return 1;
  }

  if ((digitalRead(StopSwitch) == LOW || (!flameDetector() && checkFlameout)) && firstShutdownInStartup) {
    delay(200);
    if (digitalRead(StopSwitch) == LOW || (!flameDetector() && checkFlameout)) {
      SerialBT.println("Stop switch interruption or flameout!");
      shutdown();
      return 1; // Return true if the stop switch is held, meaning previous function ends as shutdown has complited
    }
  }
  
  return 0; // Return false otherwise, meaning that previous function continues normally
}
...
void listenForCommands() {
  if (SerialBT.available()) {
    ...
    if (command.equals("FuelPrime")) {
      ...
      setOilRpm(30);
      setStarterESC(30, 0.7);
      setStarterESC(45, 0.8);
      setStarterESC(0, 1);
      sleep(2);
      setOilRpm(0);
    } 
